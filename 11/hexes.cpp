...
size_t get_distance(const std::string& input) {
    const auto path = line2vec(input);

    std::unordered_map<std::string, int> hexes;
    for (const auto& p: path) ++hexes[p];

    reduce_hexes(hexes);
    return sum_occurences(hexes);
}
...
void test() {
    ...
    ASSERT_EQ(get_distance("ne,ne,ne"), 3);
    ASSERT_EQ(get_distance("ne,ne,sw,sw"), 0);
    ASSERT_EQ(get_distance("ne,ne,s,s"), 2);
    ASSERT_EQ(get_distance("se,sw,se,sw,sw"), 3);
    ASSERT_EQ(get_distance("se,n,nw,sw,s,se,ne,nw"), 0);
}

void run() {
    std::string line;
    getline(std::cin, line);
    std::cout << get_distance(line) << "\n";
}
