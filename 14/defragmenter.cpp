...
size_t num_used(const std::string& input) {
    size_t used = 0;
    for (int i = 0; i < 128; ++i) {
        const std::string row = get_row(input, i);
        used += count(begin(row), end(row), '1');
    }
    return used;
}
//------------------------------------------------------------------------------
...
void test() {
    ...
    ASSERT_EQ(num_used("flqrgnkx"), 8108)
}

void run() {
    std::string line;
    getline(std::cin, line);
    std::cout << num_used(line) << "\n";
}
